...
\shorttitle{PyFAI}
    ...
    \aff[c]{Soleil Synchrotron, L'Orme des Merisiers, 91190 \city{Saint-Aubin},
    \country{France}} 
...
calibrant on the BioSAXS beamline BM29 at the ESRF synchrotron
...
Azimuthal regrouping and its reverse transformation (assuming
uniform-distribution over the azimuthal angle) can be performed
using pyFAI and can find use in many applications.
...
If one assumes isotropic signal (real powder without preferred
orientation), \textit{2D} diffraction patterns can be generated as shown in this
example:
...
In this snippet of code, in the second line, a reference sample LaB$_6$ is chosen 
among the known calibrants before the the wavelength is set.
...
The \textit{fake\_calibration\_image} takes more parameters to help set the U, V and W
...
measures the spacial offset between the 2D diffraction image and the
one regenerated from the integrated pattern, using a phase correlation
algorithm.
This gives a measure of the precision of localization of the PONI, which
...
PyFAI's azimuthal integrator features a \textit{separate} method for separating
the background with an azimuthal symmetry (amorphous scattering or powder's
ring), from the Bragg peaks automatically.
...
detector (image taken at the ID23-2 beamline of the ESRF). A
...
protein crystallography experiment (data from beamline ID23 at ESRF
synchrotron).}
\end{center}
\end{figure}
...
front of the X-ray beam (using a jet or moving a motor) and data is acquired
continuously, using a fast detector (from dozens of Hertz to kHz).
Those experiments produce a huge amount of data while only a small fraction of the
...
diffraction data within each frame and decides whether to save it or not.
...
into other software by the different ESRF beamlines, in order to achieve 
on-line azimuthal integration, by using either LImA or a
dedicated on-line data analysis server, like EDNA \cite{edna} in the case of the
BioSaxs beamline \cite{bm29} and DAHU for the TRUSAXS one (ID02).
...
at the Petra III synchrotron by Gunthard Benecke and co-workers) \cite{dpdak}
and \textit{Dioptas} (developed at the APS synchrotron by Clemens Prescher).
Most of those software packages offer a graphical interface to ease the
data processing for a specific type of experiment.

\section{Conclusion and Future Work}

This contribution described the improvements in the version 0.10 of the pyFAI
library, focusing on the detector representation in space, ring
...
in future versions that would allow the implementation of those new features.
...
It provides high performances in image treatment and azimuthal integration thanks
...
an issue tracker in addition to code hosting.
To ease the distribution, the
software is available as an official Debian package and is included in some of the
...
and the Soleil, Desy and APS synchrotrons did. Collaboration is encouraged and
...
the same code calculating the geometry and performing the integration was very high
in the list of the project's specifications.
...
process has also become more versatile.

After an initial, rough calibration, pyFAI can perform many operation to
...
\item \textit{fix}: prevent the parameter from being refined \textit{fix wavelength}
...
\item \textit{show}: print the current geometry parameters on the screen 
...
heavily correlated so it is important to use as many rings as possible at high angle.

The \textit{validate} option allows the user to measure the offset between the actual
...
center point of around one tenth of a pixel.
...
beamlines at the ESRF synchrotron (MX), where it is necessary for the sample
distance and the beam center to be automatically passed automatically in the 
...
reference compound.
After subsequently performing peak-picking using \textit{blob-detection}
(as described in \ref{blob}) and least square refinement of the geometry on
every input frame, distances and beam-center positions are returned as function
...
as OpenCL kernels, interfaced using PyOpenCL \cite{pyopencl}. OpenCL offers a
shared execution model effective on processors (CPU), graphics cards (GPU)
as well as accelerators like the Intel's Xeon Phi.

\subsection{Azimuthal Integration}
The direct azimuthal integration is basically a scatter operation that
requires extensive memory locking.
...
using only half of the space that the LUT previously used.
...
second versus the input image size (in semi-logarithmic scale).
This benchmark has been performed on a dual-socket Intel Xeon E5-2667 (2.9Ghz) computer
with an Nvidia Tesla K20 GPU and an Intel Xeon Phi accelerator.

\begin{figure}
\label{benchmark}
\begin{center}
\includegraphics[width=15cm]{benchmark.eps}
\caption{Benchmark of the various algorithm performing azimuthal integration in
pyFAI}
\end{center}
\end{figure}
...
\begin{itemize}
  ...
  can be processed in less the 19 milliseconds, which corresponds to a data
  processing rate of 885 Mega-pixels
  per second. This gain in performance is obtained from the collaborative
