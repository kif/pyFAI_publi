...
including preprocessing (dark subtraction, flatfield, solid angle and
polarization correction), image re-binning (geometry distortion, 1D and 2D
averaging) and the surrounding tools like the one used to
calibrate the experimental setup of a powder diffraction experiment or SAXS experiment that comprises
...
The 3 rotations (named \textit{rot1}, \textit{rot2} and \textit{rot3} in radian)
correspond to the rotations along the 3 axes around the origin (sample position) in this order:
verical axis, horizontal axis and finally beam axis.
...
\label{calib}
\begin{center}
\includegraphics[width=10cm]{calib_xpad.eps}
...
The data correspond to a lanthanum hexaboride (LaB$_6$) calibrant on the Crystal
beamline at synchrotron Soleil taken at 18?? keV on a Xpad S540 flat pixel
...
Due to gaps in the detector, the iso-$2\theta$ contour plot exhibits angle,
explaining where the incompletness of some rings}
\end{center}
\end{figure}
