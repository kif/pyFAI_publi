...
The inital refinement is obtaines from two mouse clicks on the two inner-most
rings (ring extraction using the Massif algorithm). Six rings are overlaid on
the TODO 
...
Try to get the help of a given action, like 'refine?'. Use done when finished. Most command are composed of 'action parameter value' like 'set wavelength 1e-10'.
Valid actions: set, help, get, bound, free, abort, done, refine, validate, recalib, reset, fix, show, bounds, integrate
Valid parameters: dist, poni1, poni2, rot1, rot2, rot3, wavelength
