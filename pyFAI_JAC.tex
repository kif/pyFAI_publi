...
but they can be overridden by the command line argument \textem{--gaussian=XX},
...
\textem{pyFAI-calib}, figure \ref{calib}) is provided
...
SciPy (function \textem{scipy.optimize.min_slsqp}).
...
Using a \textem{scatter to gather} transformation, the azimuthal integration for
...
The method, \textem{calcfrom1d} is available from any
\textem{AzimuthalIntegrator} or \textem{Geometry} instance. 
...
The \textem{fake\_calibration\_image} contains further option to set U, V and W
...
Calibrants in pyFAI contain only their \textem{d-spacing}, so the
...
The calibration tool, \textem{pyFAI-calib}, offers  a ``validate'' command which
...
PyFAI has been integrated into the processing software \textem{NanoPeakCell}
...
pipeline: \textem{NanoPeakCell} (developped at IBS by Nicolas Coquelle), PySAXS
...
master branch provides \textem{correct} results.

\section{Calibration tool}
\label{annex_calib}

The calibration tool of pyFAI, called \textem{pyFAI-calid} has been available
...
\item $get$: print out the value of a parameter \textem{get wavelength}
\item $set$: define the value of a parameter, \textem{set wavelength 1.54e-10} 
...
\item $free$: allow the parameter to be refined \textem{free rot1}
...
The \textem{validate} option allows us to measure the offset between the actual
...
The \textem{MX-calibrate} tool from pyFAI allows the automatic calibration of
a set of images taken at various distances using Debye-Scherrer rings of a
reference compound.
After performing subsequently peak-picking based on \textem{blob-detection}
